...
| Networks (§24/25) | `NetworksPage` /networks + `NetPins` store | pinned = state, not a second card | real logos ONLY: `tv/networks` + `watch/providers` (region from `zpopcorn-region`) + studios via `search/company` on demand; pinned sort first in-place, home rail `nets` (HOME_SECTIONS) hidden when zero pins, cards → `/company/:id`; the old fake hardcoded `PlatformsPage` is deleted. |
| Details logo (§32) | `media.images.logos` (default `append_to_response=images`) | hero | highest-voted logo renders as `.z-dlogo` above h1; missing → nothing renders (no fake brand). Anime also runs `createTop10` (`أفضل ١٠ أنمي`). |
| Discover panel | `discoverPanel({mediaType, genres, networks, types, base, onRun})` from `ui/filters.js` | movies/tv/anime | THE deep-browse entry: criteria grid (years·rating·votes·sort·status·language·region·genre, +networks for tv, +with_types & base-pin for anime) + explicit بحث button DISABLED until ≥1 criterion — no auto-query, no endless load. Tab switches carry a `seq` race-guard: superseded responses are dropped, never appended (§12). |
