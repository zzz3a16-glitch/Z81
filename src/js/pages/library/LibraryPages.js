    ...
    return `
      <div class="media-card" data-id="${media.id}" data-type="${media.media_type || 'movie'}">
        <div class="media-poster">
          ${poster ? `<img src="${poster}" alt="${title}" loading="lazy">` : `<div class="poster-placeholder">🎬</div>`}
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        const defaults = [
          ...
          { name: 'أفلام طويلة', icon: '⏱️', rule: 'مدة 150+ دقيقة', count: 67 },
      ...
      try {
        ...
        dashboard.innerHTML = `
          <div class="health-summary ${health.healthy ? 'healthy' : 'has-issues'}">
            <div class="health-icon">${health.healthy ? '✅' : '⚠️'}</div>
