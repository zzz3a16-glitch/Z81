...
function createLiveSection() {
  ...
  box.innerHTML = `
    <div style="display:flex;align-items:center;gap: var(--sp-3);margin-bottom: var(--sp-2)">
      <h3 style="font-size: var(--text-lg);font-weight:600;margin:0">البث المباشر (LIVE)</h3>
      <button class="btn btn-ghost btn-sm" data-open type="button" style="margin-inline-start:auto">إعدادات البث الكاملة</button>
    </div>
    <p style="color:var(--color-text-secondary);font-size: var(--text-sm);margin-bottom: var(--sp-4)">منصة IPTV المحلية: قوائم M3U، دليل XMLTV، مفضلة وسجل مشاهدة — كلها محفوظة على جهازك. تُدار المصادر من صفحة المصادر.</p>
    <div data-stats style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap: var(--sp-3)"></div>`;
  ...
  const paint = (st) => {
    ...
    ].map(([k, v]) => `<div style="padding: var(--sp-3);background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--r-md);text-align:center">
        <div style="font-size: var(--text-xl);font-weight:700">${v}</div><div style="font-size: var(--text-3xs);color:var(--color-text-muted)">${k}</div></div>`).join('');
  };
  import('../services/live/LiveService.js').then(({ live }) => live.init().then(() => paint(live.stats()))).catch(() => {
    grid.innerHTML = '<div style="font-size: var(--text-2xs);color:var(--color-text-muted)">تتعذر قراءة حالة البث الآن.</div>';
  });
  return box;
}
...
export async function EnhancedSettingsPage(params) {
 ...
 ${SETTINGS_CATEGORIES.map(cat => `
              <a href="/settings/${cat.id}" data-router class="settings-nav-item ${section === cat.id ? 'active' : ''}" data-section="${cat.id}" style="padding: var(--sp-3) var(--sp-3); border-radius: var(--r-md); text-decoration: none; color: var(--color-text-primary); display: flex; align-items: center; gap: var(--sp-3); transition: all 0.2s; ${section === cat.id ? 'background: var(--color-accent); color: var(--accent-contrast);' : 'background: transparent;'}">
                ...
                <div style="flex: 1; min-width: 0;">
                  ...
                  <div style="font-size: var(--text-3xs); opacity: 0.7; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${cat.desc}</div>
                </div>
              </a>
 `).join('')}
          </div>
        ...
        <div id="settings-content" style="background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-6); min-height: 500px;">
 ...
 .settings-sidebar > div { display: grid !important; grid-template-columns: repeat(auto-fill, minmax(140px, 1fr)); gap: var(--sp-2) !important; }
 }
    </style>
 `;
 ...
 content.innerHTML = `<div style="padding: var(--sp-10); text-align: center; color: var(--color-danger);">خطأ في تحميل الإعدادات: ${e.message}</div>`;
 }
 }, 50);
 ...
 div.innerHTML = `
    ...
    <div style="display: flex; flex-direction: column; gap: var(--sp-6);">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">المنطقة واللغة</h3>
        <div style="display: grid; gap: var(--sp-4);">
          <div>
            ...
            <div style="font-size: var(--text-2xs); color: var(--color-text-muted); margin-top: var(--sp-2);">تستخدم لعرض مزودي المشاهدة المتاحين</div>
          </div>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--sp-4);">
            <div>
              <label style="display: block; font-weight: 500; margin-bottom: var(--sp-2);">تنسيق الوقت</label>
            ...
            <div>
              <label style="display: block; font-weight: 500; margin-bottom: var(--sp-2);">تنسيق التاريخ</label>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">البيانات والتخزين</h3>
        <div style="display: grid; gap: var(--sp-3);">
          <div style="display: flex; justify-content: space-between; align-items: center; padding: var(--sp-3); background: var(--color-card); border-radius: var(--r-md);">
            <div>
              <div style="font-weight: 500;">التخزين المؤقت</div>
              <div style="font-size: var(--text-2xs); color: var(--color-text-muted);">بيانات TMDB المؤقتة</div>
            </div>
            <button id="clear-cache" class="btn btn-secondary btn-sm">مسح</button>
          </div>
          <div style="display: flex; justify-content: space-between; align-items: center; padding: var(--sp-3); background: color-mix(in srgb, var(--color-danger) 5%, transparent); border: 1px solid color-mix(in srgb, var(--color-danger) 20%, transparent); border-radius: var(--r-md);">
            <div>
              <div style="font-weight: 500; color: var(--color-danger);">مسح جميع البيانات</div>
              <div style="font-size: var(--text-2xs); color: var(--color-text-muted);">لا يمكن التراجع - سيتم حذف كل شيء</div>
            </div>
...
async function createAppearanceSection() {
 ...
 ${currentTheme === theme.id ? '<div style="position: absolute; top: 8px; right: 8px; background: var(--color-accent); color: var(--accent-contrast); padding: var(--sp-1) var(--sp-3); border-radius: var(--r-lg); font-size: var(--text-3xs); font-weight: 600;">' + uiIcon('check', 11) + ' نشط</div>' : ''}
          </div>
          <div style="padding: var(--sp-4);">
            <h3 style="font-weight: 600; margin-bottom: var(--sp-1); display: flex; align-items: center; gap: var(--sp-2);">${theme.name} </h3>
            <p style="font-size: var(--text-2xs); color: var(--color-text-secondary); margin-bottom: var(--sp-3); line-height: 1.5;">${theme.description}</p>
            <div style="display: flex; gap: var(--sp-2); flex-wrap: wrap;">
              <span style="font-size: var(--text-3xs); padding: 2px var(--sp-2); background: ${theme.isDark ? '#1a1a1a' : '#f0f0f0'}; color: ${theme.isDark ? 'white' : 'black'}; border-radius: var(--r-xs); border: 1px solid var(--color-border);">${theme.isDark ? 'داكن' : 'فاتح'}</span>
              <span style="font-size: var(--text-3xs); padding: 2px var(--sp-2); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-xs);">${theme.category}</span>
 ${theme.effects?.includes('glow') ? '<span style="font-size: var(--text-3xs); padding: 2px var(--sp-2); background: color-mix(in srgb, var(--accent) 10%, transparent); color: #8b5cf6; border-radius: var(--r-xs);">توهج</span>' : ''}
 ${theme.effects?.includes('glass') ? '<span style="font-size: var(--text-3xs); padding: 2px var(--sp-2); background: rgba(6,182,214,0.1); color: #06b6d4; border-radius: var(--r-xs);"> زجاج</span>' : ''}
            </div>
          </div>
        </div>
 `).join('')}
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap: var(--sp-3);margin-bottom: var(--sp-5)">
      <div style="background:var(--color-surface);border:1px solid var(--color-border);border-radius: var(--r-lg);padding: var(--sp-4)">
        <b style="font-size: var(--text-sm);display:block;margin-bottom: var(--sp-3)">كثافة الواجهة</b>
        <div style="display:flex;gap: var(--sp-2)" id="density-picker">
      ...
      <div style="background:var(--color-surface);border:1px solid var(--color-border);border-radius: var(--r-lg);padding: var(--sp-4)">
        <b style="font-size: var(--text-sm);display:block;margin-bottom: var(--sp-3)">الشريط الجانبي</b>
        <div style="display:flex;gap: var(--sp-2)" id="sidebar-pref">
    ...
    <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5); margin-bottom: var(--sp-5);">
      <h3 style="font-weight: 600; margin-bottom: var(--sp-4); display: flex; align-items: center; gap: var(--sp-2);">منشئ المظاهر المخصص</h3>
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: var(--sp-4); margin-bottom: var(--sp-4);">
        <div>
          <label style="display: block; font-size: var(--text-2xs); margin-bottom: var(--sp-2); font-weight: 500;">اللون الأساسي</label>
          <input type="color" id="custom-primary" value="#8b5cf6" class="input" style="height: 44px; padding: var(--sp-1);">
        </div>
        <div>
          <label style="display: block; font-size: var(--text-2xs); margin-bottom: var(--sp-2); font-weight: 500;">اللون الثانوي</label>
          <input type="color" id="custom-secondary" value="#06b6d4" class="input" style="height: 44px; padding: var(--sp-1);">
        </div>
        <div>
          <label style="display: block; font-size: var(--text-2xs); margin-bottom: var(--sp-2); font-weight: 500;">الخلفية</label>
          <input type="color" id="custom-bg" value="#0a0a0f" class="input" style="height: 44px; padding: var(--sp-1);">
        </div>
        <div>
          <label style="display: block; font-size: var(--text-2xs); margin-bottom: var(--sp-2); font-weight: 500;">نصف القطر</label>
    ...
    <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
      <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">إعدادات الحركة والعرض</h3>
      <div style="display: grid; gap: var(--sp-4);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <div>
            <div style="font-weight: 500; font-size: var(--text-sm);">تقليل الحركة</div>
            <div style="font-size: var(--text-2xs); color: var(--color-text-muted);">للمستخدمين الحساسين للحركة</div>
          </div>
          <input type="checkbox" id="reduced-motion" ${window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'checked' : ''} />
        </div>
        <div>
          <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">كثافة الواجهة</label>
 ...
 badge.style.cssText = 'position: absolute; top: 8px; right: 8px; background: var(--color-accent); color: var(--accent-contrast); padding: var(--sp-1) var(--sp-3); border-radius: var(--r-lg); font-size: var(--text-3xs); font-weight: 600;';
...
const playerPhaseBanner = () => `
  <div style="display:flex;gap: var(--sp-3);align-items:center;padding: var(--sp-3) var(--sp-4);margin-bottom: var(--sp-5);border-radius: var(--r-lg);
...
function createPlaybackSection() {
 ...
 ${createToggle('skip-intro', 'تخطي المقدمة', 'تخطي مقدمة المسلسل عند توفر البيانات', localStorage.getItem('zpopcorn-skip-intro') === 'true')}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">مستوى الصوت</h3>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">متابعة المشاهدة</h3>
        <div style="display: flex; align-items: center; gap: var(--sp-4); margin-bottom: var(--sp-3);">
          <label style="font-size: var(--text-sm); min-width: 180px;">عتبة متابعة المشاهدة</label>
        ...
        <div style="font-size: var(--text-2xs); color: var(--color-text-muted);">إذا شاهدت أقل من هذه النسبة، سيبدأ من البداية</div>
      </div>

      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">جودة التشغيل</h3>
    ...
    <div style="display: grid; gap: var(--sp-5);">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--sp-4);">
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">حجم الخط</label>
          ...
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">الخط</label>
          ...
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">اللون</label>
            <input type="color" id="sub-color" value="#ffffff" class="input" style="height: 40px; padding: var(--sp-1);">
          </div>
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">الخلفية</label>
            <input type="color" id="sub-bg" value="#000000" class="input" style="height: 40px; padding: var(--sp-1);">
          </div>
        </div>
      </div>

      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">الموضع والتوقيت</h3>
        <div style="display: grid; gap: var(--sp-4);">
          <div style="display: flex; align-items: center; gap: var(--sp-4);">
            <label style="min-width: 120px; font-size: var(--text-sm);">الموضع العمودي</label>
            <input type="range" id="sub-position" min="0" max="100" value="90" style="flex: 1; max-width: 200px;" />
            <span style="min-width: 40px;">90%</span>
          </div>
          <div style="display: flex; align-items: center; gap: var(--sp-4);">
            <label style="min-width: 120px; font-size: var(--text-sm);">تأخير الترجمة (ms)</label>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">اللغة الافتراضية</h3>
 ...
 ${playerPhaseBanner()}
    ...
    <div style="display: grid; gap: var(--sp-5);">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        ...
        <div style="display: grid; gap: var(--sp-4);">
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">اللغة الافتراضية</label>
          ...
          <div style="display: flex; align-items: center; gap: var(--sp-4);">
            <label style="min-width: 140px; font-size: var(--text-sm);">تأخير الصوت (ms)</label>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">التحكم</h3>
        <div style="display: grid; gap: var(--sp-3);">
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: var(--text-sm);">تطبيع الصوت</span>
            <input type="checkbox" id="audio-normalize" />
          </div>
          <div>
            <label style="font-size: var(--text-sm); margin-bottom: var(--sp-2); display: block;">قنوات الصوت</label>
 ...
 div.innerHTML = `
    ...
    <div style="display: grid; gap: var(--sp-5);">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        ...
        <div id="library-sources" style="display: grid; gap: var(--sp-2); margin-bottom: var(--sp-4);">
          <div style="padding: var(--sp-3); background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-md); display: flex; justify-content: space-between; align-items: center;">
            <div>
              <div style="font-weight: 500;">المجلد الافتراضي</div>
              <div style="font-size: var(--text-2xs); color: var(--color-text-muted);">/home/user/Videos</div>
            </div>
            <span style="font-size: var(--text-3xs); padding: 2px var(--sp-2); background: var(--color-success); color: var(--color-text-inverse); border-radius: var(--r-lg);">نشط</span>
          </div>
        </div>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">العرض</h3>
        <div style="display: grid; gap: var(--sp-4);">
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">طريقة العرض الافتراضية</label>
          ...
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">ترتيب افتراضي</label>
            <select class="input" style="max-width: 300px;">
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">الأرشيف</h3>
        <p style="font-size: var(--text-sm); color: var(--color-text-secondary); margin-bottom: var(--sp-3);">الأرشيف لا يحذف الملفات، فقط ينظمها منطقياً</p>
...
function createScannerSection() {
 ...
 ${createToggle('parallel-scan', 'فحص متوازي', 'استخدام معالجة متوازية (أسرع)', true)}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">الجدولة</h3>
        <div style="display: grid; gap: var(--sp-3);">
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">فحص مجدول</label>
          ...
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">أنواع الملفات</label>
            <input type="text" value="mp4, mkv, avi, mov, mp3, flac, m4a" class="input" style="max-width: 400px;" />
            <div style="font-size: var(--text-3xs); color: var(--color-text-muted); margin-top: var(--sp-1);">مفصولة بفواصل</div>
          </div>
        </div>
      </div>

      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">حالة الفحص</h3>
        ...
        <button class="btn btn-primary" style="margin-top: var(--sp-4); width: 100%;">فحص الآن</button>
      </div>
    </div>
 `;
 bindToggles(div);
...
async function createTMDBSection() {
 ...
 حالة الاتصال
    ...
    <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5); margin-bottom: var(--sp-5);">
      ...
      <p style="font-size: var(--text-sm); color: var(--color-text-secondary); margin-bottom: var(--sp-3);">احصل على مفتاح مجاني من <a href="https://www.themoviedb.org/settings/api" target="_blank" style="color: var(--color-accent);">themoviedb.org</a> - مطلوب لبيانات حقيقية</p>
      <div style="display: flex; gap: var(--sp-2);">
      ...
      <div style="font-size: var(--text-3xs); color: var(--color-text-muted); margin-top: var(--sp-2);">المفتاح الحالي: ${apiKey ? apiKey.slice(0, 8) + '...' + apiKey.slice(-4) : 'غير محدد (يستخدم مفتاح تجريبي)'} • التخزين: localStorage</div>
    </div>
    ...
    <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
      ...
      <div style="display: grid; gap: var(--sp-2); font-size: var(--text-sm); margin-bottom: var(--sp-4);">
        <div style="display: flex; justify-content: space-between; padding: var(--sp-2); background: var(--color-card); border-radius: var(--r-sm);"><span>البيانات الوصفية:</span><span style="font-weight: 600;">${stats.cache.metadata.size} عنصر</span></div>
        <div style="display: flex; justify-content: space-between; padding: var(--sp-2); background: var(--color-card); border-radius: var(--r-sm);"><span>البحث:</span><span style="font-weight: 600;">${stats.cache.search.size} عنصر</span></div>
        <div style="display: flex; justify-content: space-between; padding: var(--sp-2); background: var(--color-card); border-radius: var(--r-sm);"><span>عام:</span><span style="font-weight: 600;">${stats.cache.general.size} عنصر</span></div>
      </div>
      <div style="display: flex; gap: var(--sp-2); flex-wrap: wrap;">
      ...
      <div style="font-size: var(--text-3xs); color: var(--color-text-muted); margin-top: var(--sp-3);">TTL: البيانات 6 ساعات، الصور 30 يوم، البحث 1 ساعة • إستراتيجية: Stale-While-Revalidate</div>
    </div>
 `;
...
function createRecommendationsSection() {
 ...
 div.innerHTML = `
    ...
    <div style="display: grid; gap: var(--sp-5);">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        ...
        <div style="display: grid; gap: var(--sp-4);">
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">مستوى الاكتشاف</label>
            ...
            <div style="font-size: var(--text-3xs); color: var(--color-text-muted); margin-top: var(--sp-1);">يتحكم في مدى تنوع التوصيات</div>
          </div>
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">الحد الأدنى للتقييم</label>
            <input type="range" id="min-rating" min="0" max="10" step="0.5" value="6" style="max-width: 300px; width: 100%;" />
            <span id="min-rating-val" style="margin-right: var(--sp-2); font-weight: 600;">6.0</span>
          </div>
        </div>
      </div>

      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">الفلاتر</h3>
        <div style="display: grid; gap: var(--sp-3);">
 ...
 ${createToggle('include-unfinished', 'تضمين غير المكتمل', 'عرض الأعمال غير المكتملة', true)}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">الأنواع المستبعدة</h3>
        <p style="font-size: var(--text-2xs); color: var(--color-text-muted); margin-bottom: var(--sp-3);">اختر الأنواع التي لا تريد ظهورها في التوصيات</p>
        <div style="display: flex; flex-wrap: wrap; gap: var(--sp-2);">
 ${['أكشن', 'كوميديا', 'رعب', 'رومانسي', 'خيال علمي', 'دراما'].map(g => `
            <label style="display: flex; align-items: center; gap: var(--sp-2); padding: var(--sp-2) var(--sp-3); background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-xl); font-size: var(--text-2xs); cursor: pointer;">
 ...
 `).join('')}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">ملف الذوق</h3>
        <div style="font-size: var(--text-sm); color: var(--color-text-secondary); margin-bottom: var(--sp-3);">يتم بناء ملف ذوقك تلقائياً من سلوك المشاهدة</div>
        <div style="display: flex; gap: var(--sp-2);">
...
function createPrivacySection() {
    ...
    <div style="display: grid; gap: var(--sp-5);">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        ...
        <div style="font-size: var(--text-sm); color: var(--color-text-secondary); line-height: 1.8;">
          <p style="margin-bottom: var(--sp-3);">zPopcorn هو <strong>Local-First</strong> - جميع بياناتك الشخصية تبقى على جهازك:</p>
          <ul style="list-style: disc; padding-right: var(--sp-4); display: grid; gap: var(--sp-1);">
          ...
          <p style="margin-top: var(--sp-3); padding: var(--sp-3); background: color-mix(in srgb, var(--color-success) 10%, transparent); border: 1px solid color-mix(in srgb, var(--color-success) 20%, transparent); border-radius: var(--r-md);">
 ...
 لا يتم إرسال سلوك المشاهدة أو البيانات الشخصية إلى TMDB أو أي خدمة خارجية
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">الإعدادات</h3>
        <div style="display: grid; gap: var(--sp-3);">
 ...
 ${createToggle('usage-stats', 'إحصائيات الاستخدام', 'مساعدتنا على التحسين', false)}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">إدارة البيانات</h3>
        <div style="display: flex; gap: var(--sp-2); flex-wrap: wrap;">
...
async function createBackupSection() {
 ...
 div.innerHTML = `
    ...
    <div style="display: grid; gap: var(--sp-5);">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        ...
        <p style="font-size: var(--text-sm); color: var(--color-text-secondary); margin-bottom: var(--sp-4);">احفظ جميع بياناتك في ملف JSON آمن. جميع البيانات محلية.</p>
        ...
        <div style="font-size: var(--text-3xs); color: var(--color-text-muted); margin-top: var(--sp-3);">التنسيق: JSON مع schemaVersion وفحص سلامة • الملف: zpopcorn-backup-YYYY-MM-DD.json</div>
      </div>
      
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">استعادة نسخ احتياطي</h3>
        <p style="font-size: var(--text-sm); color: var(--color-text-secondary); margin-bottom: var(--sp-4);">استعد بياناتك من ملف نسخ احتياطي. سيتم إنشاء نسخة آمنة تلقائياً قبل الاستعادة.</p>
        <div style="background: color-mix(in srgb, var(--color-warning) 10%, transparent); border: 1px solid color-mix(in srgb, var(--color-warning) 20%, transparent); border-radius: var(--r-md); padding: var(--sp-3); margin-bottom: var(--sp-4); font-size: var(--text-2xs);">
 ...
 7. في حالة الفشل: استرجاع تلقائي
        ...
        <input type="file" id="restore-file" accept=".json" style="margin-bottom: var(--sp-3); padding: var(--sp-2); border: 1px solid var(--color-border); border-radius: var(--r-md); width: 100%;" />
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: var(--sp-2); font-size: var(--text-sm);">
 ${Object.entries(stats).map(([store, count]) => `
            <div style="display: flex; justify-content: space-between; padding: var(--sp-3); background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-md);">
              <span style="font-size: var(--text-2xs);">${store}</span>
 ...
 `).join('') || '<div style="color: var(--color-text-muted);">لا توجد بيانات</div>'}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">النسخ التلقائي</h3>
        <div style="display: grid; gap: var(--sp-3);">
 ${createToggle('auto-backup', 'نسخ احتياطي تلقائي', 'إنشاء نسخة يومياً تلقائياً', false)}
          <div>
            <label style="font-size: var(--text-sm); font-weight: 500; margin-bottom: var(--sp-2); display: block;">موقع النسخ</label>
 ...
 div.querySelector('#restore-info').innerHTML = `
        <div style="background: var(--color-card); padding: var(--sp-3); border-radius: var(--r-md); border: 1px solid var(--color-border);">
          <div style="font-weight: 600; margin-bottom: var(--sp-2); color: var(--color-success);">ملف صالح</div>
          <div style="display: grid; gap: var(--sp-1); font-size: var(--text-2xs); font-family: monospace;">
 ...
 div.querySelector('#restore-info').innerHTML = `<div style="color: var(--color-danger); background: color-mix(in srgb, var(--color-danger) 10%, transparent); padding: var(--sp-3); border-radius: var(--r-md); border: 1px solid color-mix(in srgb, var(--color-danger) 20%, transparent);">خطأ: ${error.message}</div>`;
...
function createKeyboardSection() {
 ...
 ${createShortcut('↑ ↓', 'رفع/خفض الصوت')}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);"> التنقل</h3>
        <div style="display: grid; gap: var(--sp-2); font-size: var(--text-sm);">
 ...
 ${createShortcut('G then T', 'الذهاب للمسلسلات')}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">تخصيص</h3>
        <p style="font-size: var(--text-2xs); color: var(--color-text-muted); margin-bottom: var(--sp-3);">يمكنك تخصيص الاختصارات قريباً</p>
...
function createShortcut(keys, desc) {
 return `
    <div style="display: flex; justify-content: space-between; align-items: center; padding: var(--sp-3) var(--sp-3); background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-md);">
      <span>${desc}</span>
      <kbd style="padding: var(--sp-1) var(--sp-2); background: var(--color-surface); border: 1px solid var(--color-border); border-bottom-width: 2px; border-radius: var(--r-xs); font-family: monospace; font-size: var(--text-3xs); font-weight: 600;">${keys}</kbd>
    </div>
 `;
}
...
async function createAdvancedSection() {
 ...
 div.innerHTML = `
    ...
    <div style="display: grid; gap: var(--sp-5);">
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">وضع المطور</h3>
        <div style="display: grid; gap: var(--sp-3);">
 ...
 ${createToggle('console-logs', 'سجلات وحدة التحكم', 'حفظ السجلات في localStorage', false)}
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);">الأداء</h3>
        <div style="display: grid; gap: var(--sp-2); font-size: var(--text-2xs); font-family: monospace;">
          <div style="display: flex; justify-content: space-between; padding: var(--sp-2); background: var(--color-card); border-radius: var(--r-sm);"><span>FCP:</span><span>${perfReport.metrics.fcp ? perfReport.metrics.fcp.toFixed(2) + 'ms' : 'غير متاح'}</span></div>
          <div style="display: flex; justify-content: space-between; padding: var(--sp-2); background: var(--color-card); border-radius: var(--r-sm);"><span>LCP:</span><span>${perfReport.metrics.lcp ? perfReport.metrics.lcp.toFixed(2) + 'ms' : 'غير متاح'}</span></div>
          <div style="display: flex; justify-content: space-between; padding: var(--sp-2); background: var(--color-card); border-radius: var(--r-sm);"><span>CLS:</span><span>${perfReport.metrics.cls?.toFixed(4) || '0'}</span></div>
          <div style="display: flex; justify-content: space-between; padding: var(--sp-2); background: var(--color-card); border-radius: var(--r-sm);"><span>الذاكرة:</span><span>${perfReport.memory ? (perfReport.memory.used / 1024 / 1024).toFixed(1) + ' MB' : 'غير متاح'}</span></div>
        </div>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-4);"> قاعدة البيانات</h3>
        <div style="display: grid; gap: var(--sp-2); margin-bottom: var(--sp-4);">
        ...
        <div id="db-check-result" style="font-size: var(--text-2xs);"></div>
      </div>

      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">الأمان</h3>
        <div style="display: flex; gap: var(--sp-2); flex-wrap: wrap;">
        ...
        <div id="sec-logs" style="margin-top: var(--sp-3); font-size: var(--text-3xs); font-family: monospace; max-height: 200px; overflow-y: auto; background: var(--color-card); padding: var(--sp-3); border-radius: var(--r-md); display: none;"></div>
      </div>
    </div>
 `;
 ...
 logsDiv.innerHTML = logs.length ? logs.map(l => `<div style="padding: var(--sp-1) 0; border-bottom: 1px solid var(--color-border);">${new Date(l.timestamp).toLocaleString()} - ${l.event}: ${JSON.stringify(l.details)}</div>`).join('') : 'لا توجد سجلات';
 } catch {
...
function createAboutSection() {
 ...
 div.innerHTML = `
    ...
    <div style="text-align: center; padding: var(--sp-8) 0; border-bottom: 1px solid var(--color-border); margin-bottom: var(--sp-6);">
      ...
      <div style="display: inline-flex; gap: var(--sp-2); margin-top: var(--sp-4);">
        <span style="padding: var(--sp-1) var(--sp-3); background: var(--color-success); color: var(--color-text-inverse); border-radius: var(--r-xl); font-size: var(--text-3xs); font-weight: 600;">جاهز للإنتاج</span>
        <span style="padding: var(--sp-1) var(--sp-3); background: var(--color-accent); color: var(--accent-contrast); border-radius: var(--r-xl); font-size: var(--text-3xs); font-weight: 600;">${uiIcon('check', 11)} TMDB حقيقي</span>
        <span style="padding: var(--sp-1) var(--sp-3); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-xl); font-size: var(--text-3xs);"> SA</span>
      </div>
    </div>
    
    <div style="display: grid; gap: var(--sp-5);">
      <div style="background: linear-gradient(135deg, color-mix(in srgb, var(--accent) 10%, transparent), rgba(6,182,214,0.1)); border: 1px solid color-mix(in srgb, var(--accent) 20%, transparent); border-radius: var(--r-lg); padding: var(--sp-5);">
        <h3 style="font-weight: 700; margin-bottom: var(--sp-3); display: flex; align-items: center; gap: var(--sp-2);">الرؤية</h3>
        <p style="font-size: var(--text-sm); color: var(--color-text-secondary); line-height: 1.8;">
 تحويل zPopcorn من مكتبة وسائط محلية ذكية إلى <strong>مساعد وسائط شخصي ذكي متكامل</strong> يفهم تفضيلاتك، يتنبأ بسلوك المشاهدة، ويقدم تجربة سينمائية فاخرة مع قدرة عمل كاملة دون اتصال.
      ...
      <div>
        <h3 style="font-weight: 600; margin-bottom: var(--sp-3);">المميزات الرئيسية</h3>
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(250px, 1fr)); gap: var(--sp-2);">
 ${[
 ...
 'SQLite + IndexedDB + localStorage مع ترحيل آمن'
            ...
            <div style="display: flex; align-items: flex-start; gap: var(--sp-2); padding: var(--sp-3) var(--sp-3); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-md); font-size: var(--text-2xs);">
 ...
 ${['Electron.js', 'Node.js', 'Vite 5.2', 'Vanilla JS ES2024', 'SQLite', 'IndexedDB', 'TMDB API', 'mpv Player', 'Socket.IO 4.7', 'PWA', 'RTL', 'CSS Variables', 'Design Tokens'].map(t => `
            <span style="padding: var(--sp-2) var(--sp-3); background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-2xl); font-size: var(--text-3xs); font-weight: 500; font-family: monospace;">${t}</span>
 `).join('')}
        </div>
      </div>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--sp-4);">
        <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
          <h3 style="font-weight: 600; margin-bottom: var(--sp-2); font-size: var(--text-sm);">الأرشفة</h3>
          <p style="font-size: var(--text-2xs); color: var(--color-text-secondary); line-height: 1.6;">المنصة المحلية الأولى - جميع البيانات الشخصية محلية. TMDB للبيانات الوصفية فقط.</p>
        </div>
        <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg); padding: var(--sp-5);">
          <h3 style="font-weight: 600; margin-bottom: var(--sp-2); font-size: var(--text-sm);">TMDB Attribution</h3>
          <p style="font-size: var(--text-3xs); color: var(--color-text-secondary); line-height: 1.6;">
 يستخدم TMDB API لكنه غير معتمد من TMDB.<br>
      ...
      <div style="text-align: center; padding: var(--sp-6); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg);">
        <p style="font-size: var(--text-sm); font-weight: 600; margin-bottom: var(--sp-1);">صُنع بحبٍّ للمجتمع العربي</p>
        <p style="font-size: var(--text-2xs); color: var(--color-text-muted);">المنطقة الافتراضية: السعودية (SA) • اللغة: العربية RTL • تصميم: Apple-Level</p>
        <p style="font-size: var(--text-3xs); color: var(--color-text-muted); margin-top: var(--sp-3); font-family: monospace;">© 2026 zPopcorn Ultimate v2.0.0 • Production-Grade Media Intelligence Platform<br>Architecture: Electron + Node + Vite + Vanilla JS ES2024 + mpv + Socket.IO</p>
        <div style="margin-top: var(--sp-4); display: flex; justify-content: center; gap: var(--sp-2);">
          <a href="https://github.com" target="_blank" style="padding: var(--sp-2) var(--sp-3); background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-xl); font-size: var(--text-3xs); text-decoration: none; color: var(--color-text-secondary);">GitHub</a>
          <a href="https://www.themoviedb.org/" target="_blank" style="padding: var(--sp-2) var(--sp-3); background: var(--color-card); border: 1px solid var(--color-border); border-radius: var(--r-xl); font-size: var(--text-3xs); text-decoration: none; color: var(--color-text-secondary);">TMDB</a>
          <span style="padding: var(--sp-2) var(--sp-3); background: var(--color-success); color: var(--color-text-inverse); border-radius: var(--r-xl); font-size: var(--text-3xs); font-weight: 600;">جاهز للإنتاج</span>
        </div>
      </div>
    </div>
 `;
...
function createToggle(id, title, desc, checked) {
 return `
    <div style="display: flex; justify-content: space-between; align-items: center; padding: var(--sp-4); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--r-lg);">
      <div style="flex: 1;">
        <div style="font-weight: 500; font-size: var(--text-sm);">${title}</div>
        <div style="font-size: var(--text-3xs); color: var(--color-text-muted); margin-top: 2px;">${desc}</div>
      </div>
      <label style="position: relative; display: inline-block; width: 44px; height: 24px; flex-shrink: 0; cursor: pointer; margin-right: var(--sp-3);">
