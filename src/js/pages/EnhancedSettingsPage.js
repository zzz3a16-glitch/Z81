...
const SETTINGS_CATEGORIES = [
  { id: 'general', name: 'عام', icon: '⚙️', desc: 'الإعدادات العامة' },
  { id: 'appearance', name: 'المظهر', icon: '🎨', desc: 'الثيمات والعرض' },
  { id: 'playback', name: 'التشغيل', icon: '▶️', desc: 'إعدادات المشغل' },
  ...
  { id: 'watch-party', name: 'المشاهدة الجماعية', icon: '👥', desc: 'غرف المشاهدة' },
  { id: 'privacy', name: 'الخصوصية', icon: '🔒', desc: 'البيانات والخصوصية' },
  { id: 'backup', name: 'النسخ الاحتياطي', icon: '💾', desc: 'حفظ واستعادة' },
  { id: 'keyboard', name: 'لوحة المفاتيح', icon: '⌨️', desc: 'اختصارات' },
  { id: 'advanced', name: 'متقدم', icon: '🔧', desc: 'إعدادات متقدمة' },
  { id: 'about', name: 'حول', icon: 'ℹ️', desc: 'معلومات التطبيق' }
];
...
export async function EnhancedSettingsPage(params) {
  ...
  container.innerHTML = `
    <div class="container" style="padding-top: 24px; padding-bottom: 40px;">
      <h1 style="font-size: 2rem; font-weight: 700; margin-bottom: 8px;">⚙️ الإعدادات</h1>
    ...
    try {
      ...
      switch (section) {
        ...
        case 'notifications':
          ...
          break;
        case 'watch-party':
          sectionEl = createWatchPartySection();
          break;
        case 'privacy':
  ...
  div.innerHTML = `
      ...
      ${allThemes.map(theme => `
        <div class="theme-card ${currentTheme === theme.id ? 'active' : ''}" data-theme="${theme.id}" style="border: 2px solid ${currentTheme === theme.id ? 'var(--color-accent)' : 'var(--color-border)'}; border-radius: 12px; overflow: hidden; cursor: pointer; transition: all 0.2s; background: var(--color-surface); position: relative;">
          ...
          <div style="padding: 16px;">
            <h3 style="font-weight: 600; margin-bottom: 4px; display: flex; align-items: center; gap: 8px;">${theme.name} ${theme.isDark ? '🌙' : '☀️'}</h3>
    ...
    <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px; margin-bottom: 20px;">
      ...
      <div style="display: flex; gap: 8px; flex-wrap: wrap;">
        <button id="preview-custom" class="btn btn-secondary">👁️ معاينة مباشرة</button>
  ...
  div.innerHTML = `
    ...
    <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px; margin-bottom: 20px;">
      ...
      <div style="display: flex; gap: 8px;">
        ...
        <button id="toggle-api-key" class="btn btn-ghost">👁️</button>
      </div>
...
function createWatchPartySection() {
  const div = document.createElement('div');
  div.innerHTML = `
    <h2 style="font-size: 1.5rem; font-weight: 600; margin-bottom: 20px;">المشاهدة الجماعية</h2>
    <div style="display: grid; gap: 20px;">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px;">
        <h3 style="font-weight: 600; margin-bottom: 12px;">🎬 كيفية العمل</h3>
        <ol style="font-size: 13px; color: var(--color-text-secondary); line-height: 1.8; padding-right: 16px; list-style: decimal;">
          <li>أنشئ غرفة مشاهدة جماعية</li>
          <li>شارك الرابط مع أصدقائك</li>
          <li>شاهدوا معاً مع مزامنة تلقائية</li>
          <li>دردشة مباشرة أثناء المشاهدة</li>
        </ol>
      </div>

      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px;">
        <h3 style="font-weight: 600; margin-bottom: 16px;">إعدادات الخادم</h3>
        <div style="display: grid; gap: 12px;">
          <div>
            <label style="font-size: 13px; font-weight: 500; margin-bottom: 6px; display: block;">عنوان الخادم</label>
            <input type="text" value="http://localhost:3001" class="input" style="max-width: 400px;" />
          </div>
          ${createToggle('wp-enabled', 'تفعيل المشاهدة الجماعية', 'السماح بإنشاء غرف مشاهدة', true)}
        </div>
      </div>

      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px;">
        <h3 style="font-weight: 600; margin-bottom: 12px;">الخصوصية</h3>
        <div style="font-size: 12px; color: var(--color-text-secondary); line-height: 1.6; background: var(--color-card); padding: 12px; border-radius: 8px; border: 1px solid var(--color-border);">
          <strong>🔒 لا يتم إرسال:</strong> ملفاتك المحلية، مسارات الملفات، بياناتك الشخصية<br>
          <strong>✅ يتم مزامنة:</strong> هوية الوسائط، حالة التشغيل، الموضع، رسائل الدردشة<br>
          <strong>⚠️ ملاحظة:</strong> إذا كان الملف محلي فقط عندك، الآخرون لا يستطيعون تشغيله
        </div>
      </div>

      <button class="btn btn-primary" style="width: 100%;">🎬 إنشاء غرفة مشاهدة</button>
    </div>
  `;
  bindToggles(div);
  return div;
    ...
    <div style="display: grid; gap: 20px;">
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 8px;">
          ...
          <button id="backup-settings" class="btn btn-secondary">⚙️ الإعدادات فقط</button>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px;">
        <h3 style="font-weight: 600; margin-bottom: 12px;">♻️ استعادة نسخ احتياطي</h3>
  ...
  div.innerHTML = `
    ...
    <div style="display: grid; gap: 20px;">
      <div style="background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.3); border-radius: 12px; padding: 16px; font-size: 13px;">
        ⚠️ <strong>تحذير:</strong> هذه إعدادات متقدمة للمطورين. التغيير الخاطئ قد يؤثر على التطبيق.
      </div>
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px;">
        <h3 style="font-weight: 600; margin-bottom: 16px;">🗄️ قاعدة البيانات</h3>
...
function createAboutSection() {
  ...
  div.innerHTML = `
    ...
    <div style="display: grid; gap: 20px;">
      ...
      <div style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 20px;">
        <h3 style="font-weight: 600; margin-bottom: 12px;">🛠️ التقنيات</h3>
        <div style="display: flex; flex-wrap: wrap; gap: 8px;">
      ...
      <div style="text-align: center; padding: 24px; background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px;">
        <p style="font-size: 14px; font-weight: 600; margin-bottom: 4px;">صُنع بـ ❤️ للمجتمع العربي</p>
