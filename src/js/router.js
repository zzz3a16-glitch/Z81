...
export class Router {
  ...
  async navigate(path, params = {}, options = {}) {
    ...
    // Update history (hash mode on file:// — packaged desktop — spec 52)
    const hashMode = window.__zpopcornHashMode === true;
    if (hashMode) {
      if (replace) window.location.replace('#' + fullPath);
      else window.location.hash = fullPath;
      if (!replace) this.history.push({ path, params, query, timestamp: Date.now() });
    } else if (replace) {
  ...
  init() {
    ...
    window.addEventListener('popstate', (event) => {
      ...
      this.handleRoute(currentPath(), state.params || {}, state.query || {});
    });
    if (hashMode) {
